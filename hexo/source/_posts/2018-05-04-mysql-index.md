...
完全二叉树：只有最底层不满，最底层从左侧开始没有空的。(**Complete Binary Tree**)

##### 二分查找树

也叫二叉搜索树（BST树），首先是二叉树，小的存储到左侧节点，大的存储到右侧节点。

- 左子树上所有结点的值均小于它的根结点的值
- 右子树上所有结点的值均大于它的根结点的值

查找的时候，从根节点开始比较，小于根的值就递归从左子树中查找，大于根的值就递归从右子树中查找。

添加节点也是一样，从根节点开始，找到合适的问题并添加。

删除节点麻烦一些，如果删除叶子节点比较简单，直接删除即可；如果删除非叶子节点，

##### AVL树

高度平衡二叉树，在AVL树中任何节点的两个子树的高度最大差别为一。

O(log n)

##### 红黑树

JDK8的HashMap用到了红黑树来存储数据。

从二叉树看，红黑树是一颗相对平衡的二叉树。二叉树-->搜索二叉树-->平衡搜索二叉树--> 红黑树。
...
![B树例子](/images/mysql-index-b-tree-e1.png)

#### B+树

B树有很多变种，B+树就是其中一种，MySQL使用B+树作为索引的数据结构。
...
- 每个叶子节点增加一个指向相邻叶子节点的指针；
- 这样可以提高区间查询的效率，例如：where age >= 30 and age < 50；
- ​
